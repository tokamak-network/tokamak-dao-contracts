...
const POWERTON_SEIG_RATE = _WTON('0.05');
...
describe('Test 1', function () {
  ...
  describe('Execute', function () {
    ...
    describe(`Agendas`, async function () {
      ...
      describe(`PowerTONSwapper`, async function () {
        it(`agenda executing`, async function () {
          const testAddress = "0x0000000000000000000000000000000000000007";
          const testRate = "100000000000000000000000000";

          const powerton1 = await seigManager.powerton();
          const powerTONSeigRate1 = await seigManager.powerTONSeigRate();
          ...
          const receipt = await ton.approveAndCall(
          ...
          //console.log(`TEST tx: ${JSON.stringify(tx)}`);
          ...
          const powerton2 = await seigManager.powerton();
          const powerTONSeigRate2 = await seigManager.powerTONSeigRate();

          expect(powerton2).to.not.equal(powerton1);
          expect(powerton2).to.equal(testAddress);

          expect(powerTONSeigRate2).to.not.equal(powerTONSeigRate1);
          expect(powerTONSeigRate2).to.bignumber.equal(toBN(testRate));
        });
      });
    });
  });
