...
contract DAOCommitteeStore is OwnableAdmin{
    ...
    constructor(address _ton) public { 
        ...
        ton = _ton;
        maxCommittees = 3; 
        if(members.length==0)  members.push( Member(address(0),address(0),'',now, 0,0,0,0 ) );
    ...
    function isCommittee(address _operator) public view returns (bool _iscommittee, uint256 _committeeIndex) { 
        if( _operator != address(0) && memberId[_operator] != 0 && committeeId[_operator] !=0  ) return (true,committeeId[_operator]);
        else return (false,0);  
    }  
    
    function existsMember(address _operator) public view returns (bool) { 
        if( _operator != address(0) && memberId[_operator] != 0  ) return true;
        else return false;  
    }  
    ...
    function detailedMember(address user) public view returns (uint256 _committeeId, address layer2,  address operaor, string memory name, uint memberSince , uint256 castingcount) {
